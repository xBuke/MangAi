(function () {
  ...
  function renderPost(post) {
    ...
    card.innerHTML =
      ...
      'Updated: ' + escapeHtml(dateFormatted) +
      ' · ' +
      escapeHtml(post.reading_time_minutes + ' min read') +
      '</p>' +
  ...
  fetch('posts.json')
    ...
    .then(function (posts) {
      ...
      if (!Array.isArray(posts) || posts.length === 0) {
        listEl.innerHTML = '<p class="blog-loading">No posts published yet.</p>';
        return;
      }
    ...
    .catch(function () {
      if (loadingEl) loadingEl.remove();
      listEl.innerHTML = '<p class="blog-loading">Error loading posts. Please refresh the page.</p>';
    });
})();
